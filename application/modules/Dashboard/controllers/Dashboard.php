...
class Dashboard extends CI_Controller {
    ...
    public function pool_entry($user_id, $pool_level = 1, $pool_amount = 500) {
        $pool_upline = $this->User_model->get_single_record('tbl_pool', array('level1 <' => 5, 'pool_level' => $pool_level), 'id,user_id,level1');
    ...
    public function paypal_transaction_status($topup_id, $transaction_id) {
        $result['success'] = 0;
        $user_id = $this->session->userdata['user_id'];
        $curl = curl_init();
        curl_setopt_array($curl, array(
            ...
            curl_setopt($curl, CURLOPT_USERPWD, $this->config->item('client_id') . ":" . $this->config->item('secret')),
            CURLOPT_HTTPHEADER => array(
        ...
        $curl = curl_init();
        curl_setopt_array($curl, array(
            CURLOPT_URL => "https://api.sandbox.paypal.com/v2/checkout/orders/" . $transaction_id,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
        ...
        if ($transaction_response['status'] == 'COMPLETED') {
            $transaction = $this->User_model->get_single_record('paypal_transactions', array('transaction_id' => $transaction_id), '*');
            if (empty($transaction)) {
                $transactionArr = array(
                    'user_id' => $user_id,
                    'amount' => $transaction_response['purchase_units'][0]['amount']['value'],
                    'transaction_id' => $transaction_id,
                    'activate_user_id' => $topup_id,
                    'payment_status' => $transaction_response['status'],
                );
                $this->User_model->add('paypal_transactions', $transactionArr);
                $package = $this->User_model->get_single_record('tbl_package', array('price' => $transaction_response['purchase_units'][0]['amount']['value']), '*');
                if (!empty($package)) {
                    /* user activation code */
                    $user = $this->User_model->get_single_record('tbl_users', array('user_id' => $topup_id), 'user_id,sponser_id,paid_status');
                    if ($user['paid_status'] == 0) {
                        ...
                        $this->User_model->update('tbl_users', array('user_id' => $topup_id), $topupData);
                        ...
                        $DirectIncome = array(
                            ...
                            'amount' => $package['direct_income'],
                            'type' => 'direct_income',
                            'description' => 'Direct Income from Activation of Member ' . $topup_id,
                        );
                        $this->User_model->add('tbl_income_wallet', $DirectIncome);
                        $this->update_business($user['user_id'], $user['user_id'], $level = 1, $package['bv'], $type = 'topup');
                        $roiArr = array(
                        ...
                        $result['message'] = 'This Account Already Activate';
                    }
                } else {
                    $result['message'] = 'Invalid Package Selection';
                }
            } else {
                $result['message'] = 'This is an Old Transaction';
            }
        }
