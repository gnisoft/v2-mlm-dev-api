    ...
    public function Profile($mode = 'profile') {
        if (is_logged_in()) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                $Userdata['address'] = $data['address'];
                $Userdata['address2'] = $data['address2'];
...
//            pr($response);
    ...
    public function check_topup_user($user_id) {
        $response = array();
        $response['success'] = 0;
        $user = $this->User_model->get_single_record('tbl_users', array('user_id' => $user_id), 'id,user_id,paid_status,name');
        if (!empty($user)) {
            if ($user['paid_status'] == 0) {
                $response['success'] = 1;
                $response['message'] = $user['name'];
            } else {
                $response['message'] = 'This User Account Already Activated';
            }
        } else {
